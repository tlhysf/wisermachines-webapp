...
import {
  editZoneAction,
  deleteZoneAction,
} from "../../../../../redux/actions/machineMonitoring/zonesActions";
...
const EditZone = (props) => {
  ...
  const editLoading = useSelector((state) => state.zones.editZoneLoading);
  const deleteLoading = useSelector((state) => state.zones.deleteZoneLoading);
  ...
  const handleSave = (e) => {
    ...
    if (formData.name && isNotEmpty(formData.name)) {
      const requestBody = requestBodyFormat.editZone;
      requestBody.id = openForm.ID;
      requestBody.name = formData.name;

      editZoneAction(dispatch, requestBody);
    } else {
      setErrors({
        ...errors,
  ...
  const handleDelete = (e) => {
    ...
    if (window.confirm("Are you sure you want to delete this item?")) {
      const requestBody = requestBodyFormat.deleteZone;
      requestBody.id = openForm.ID;

      deleteZoneAction(dispatch, requestBody);
    }
  };
